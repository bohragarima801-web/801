...
export default function HeroSliderPage() {
  ...
  async function loadSlides() {
    try {
      const res = await fetch('/api/admin/hero?placement=PRODUCT')
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      try {
        ...
        throw new Error(res.status === 413 ? 'Image size is too large' : `Server Error: ${res.statusText}`)
      }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      const url = editingId ? `/api/admin/hero?id=${editingId}` : '/api/admin/hero'
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Product Page Hero Slider"
        description="Configure rotating hero banners and sliders for the main products page."
        breadcrumbs={[{ label: 'CMS', href: '/admin/cms' }, { label: 'Product Slider' }]}
        action={{
