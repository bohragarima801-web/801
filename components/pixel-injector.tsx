...
export function PixelInjector() {
  ...
  useEffect(() => {
    async function loadSettings() {
      try {
        // Use public endpoint — no auth required, no 401 errors
    ...
    // Delay pixel loading until browser is idle — improves Lighthouse performance score
    if (typeof window !== 'undefined' && 'requestIdleCallback' in window) {
      (window as any).requestIdleCallback(() => loadSettings(), { timeout: 3000 })
    } else {
